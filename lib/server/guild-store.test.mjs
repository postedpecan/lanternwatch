...
const { getCatalogDependencies, getDashboard, getHistory, recordGuildEvent } = await import("./guild-store.ts");
...
test("token receipts are additive, repeat-safe, and retain honest coverage for legacy history", async (t) => {
  const directory = await mkdtemp(path.join(tmpdir(), "lanternwatch-token-usage-"));
  const previousDatabase = process.env.LANTERNWATCH_DB_PATH;
  process.env.LANTERNWATCH_DB_PATH = path.join(directory, "tokens.db");
  closeStore();
  t.after(async () => {
    closeStore();
    if (previousDatabase === undefined) delete process.env.LANTERNWATCH_DB_PATH;
  ...
  const first = event({
    eventId: "usage-start", runId: "usage-run", agent: "frontend-engineer", agentInstanceId: "frontend-1",
    usage: { source: "codex", model: "test", mode: "cumulative", inputTokens: 10, outputTokens: 20, totalTokens: 30 },
  });
  recordGuildEvent(first);
  recordGuildEvent({ ...first, usage: { source: "codex", model: "test", mode: "delta", inputTokens: 999, totalTokens: 999 } });
  recordGuildEvent(event({
    eventId: "usage-update", runId: "usage-run", agent: "frontend-engineer", agentInstanceId: "frontend-1", status: "complete", runComplete: true,
    usage: { source: "codex", model: "test", mode: "cumulative", inputTokens: 15, outputTokens: 30, totalTokens: 45 },
  }));
  recordGuildEvent(event({
    eventId: "usage-components-only", runId: "usage-run", agent: "data-engineer", agentInstanceId: "data-1",
    usage: { source: "codex", mode: "delta", inputTokens: 5, cachedInputTokens: 2, outputTokens: 8 },
  }));
  recordGuildEvent(event({ eventId: "usage-legacy", runId: "usage-run", agent: "qa-engineer", agentInstanceId: "qa-1" }));

  const dashboard = getDashboard();
  assert.deepEqual(dashboard.statistics.tokenUsage, {
    inputTokens: 20, cachedInputTokens: 2, outputTokens: 38, reasoningTokens: null, totalTokens: 45,
    reportedAgentInstances: 2, unreportedAgentInstances: 1,
  });
  assert.equal(dashboard.agentMetrics["frontend-engineer"].tokenUsage?.totalTokens, 45);
  assert.equal(dashboard.agentMetrics["data-engineer"].tokenUsage?.totalTokens, null, "components must not be presented as a fabricated total");
  assert.equal(dashboard.runs[0].tokenUsage?.unreportedAgentInstances, 1);
});

test("history query filters server-side and returns stable keyset pages", async (t) => {
  const directory = await mkdtemp(path.join(tmpdir(), "lanternwatch-history-query-"));
  const previousDatabase = process.env.LANTERNWATCH_DB_PATH;
  process.env.LANTERNWATCH_DB_PATH = path.join(directory, "history.db");
  closeStore();
  t.after(async () => {
    closeStore();
    if (previousDatabase === undefined) delete process.env.LANTERNWATCH_DB_PATH;
  ...
  const alpha = recordGuildEvent(event({ eventId: "history-alpha", runId: "history-alpha", quest: "Find amber visual bugs", message: "Searchable amber trail", agent: "frontend-engineer", status: "complete", runComplete: true }));
  recordGuildEvent(event({ eventId: "history-beta", runId: "history-beta", quest: "Migrate tokens", message: "Data migration", agent: "data-engineer", status: "complete", runComplete: true }));
  recordGuildEvent(event({ eventId: "history-gamma", runId: "history-gamma", quest: "Review report", message: "Interrupted review", agent: "qa-engineer", status: "interrupted" }));
  const database = new DatabaseSync(process.env.LANTERNWATCH_DB_PATH);
  database.prepare("UPDATE runs SET updated_at = ? WHERE id = ?").run("2026-01-03T00:00:00.000Z", alpha.runId);
  database.prepare("UPDATE runs SET updated_at = ? WHERE source_run_id = ?").run("2026-01-02T00:00:00.000Z", "history-beta");
  database.prepare("UPDATE runs SET updated_at = ? WHERE source_run_id = ?").run("2026-01-01T00:00:00.000Z", "history-gamma");
  database.close();

  const searched = getHistory({ q: "amber", limit: 25 });
  assert.equal(searched.totalMatches, 1);
  assert.equal(searched.items[0].sourceRunId, "history-alpha");
  const completed = getHistory({ status: "completed", agent: "data-engineer", limit: 25 });
  assert.equal(completed.totalMatches, 1);
  assert.equal(completed.items[0].sourceRunId, "history-beta");
  const firstPage = getHistory({ limit: 1 });
  assert.equal(firstPage.items.length, 1);
  assert.ok(firstPage.nextCursor);
  const secondPage = getHistory({ limit: 1, cursor: firstPage.nextCursor });
  assert.equal(secondPage.items.length, 1);
  assert.notEqual(secondPage.items[0].id, firstPage.items[0].id);
  assert.throws(() => getHistory({ projectId: "missing", limit: 25 }), /invalid-project/);
});
